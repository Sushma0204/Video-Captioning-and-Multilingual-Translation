...
const Telugu = () => {
  ...
  const handleTranslate = async () => {
    ...
    setLoading(true)
    try {
      const response = await fetch('http://127.0.0.1:5000/translate-telugu', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="flex justify-center items-center min-h-[58vh] px-8 py-2 space-x-8 mt-4">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-xl w-1/3 h-92 flex flex-col">
        <h2 className="text-xl font-semibold text-green-900 mb-4">Enter Text to Translate (Telugu)</h2>
        <textarea
          className="w-full h-full p-4 border border-gray-300 rounded-lg resize-none focus:outline-green-500"
          placeholder="Type your text here to translate to Telugu..."
          value={inputText}
        ...
        <button
          ...
          disabled={loading}
        >
          {loading ? 'Translating...' : 'Translate to Telugu'}
        </button>
      </div>
